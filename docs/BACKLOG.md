...
- [x] **2 — Read error handling.** ~35 reads ignored `error`, rendering failures as empty data. Every table read now checks `error` and keeps existing state on failure, toasting where a `showToast` prop exists (773ff4c; the delete-guard bug it uncovered in `ExerciseDatabase.requestDelete` is 95e3f7c). Deliberate exception: the two supplementary stats reads in `routineMeta.js` (`session_exercises`, `workout_sessions`) still degrade silently — a routine tile renders fine without "last performed"/avg, and the module has no toast. Verified offline in-browser; see the investigation note below.
  - **Offline refetch pattern — not our bug (investigated across 3 sessions, closed).** Going offline appeared to make ~20 reads re-fire repeatedly with no code path explaining it: our `[DBG]` instrumentation proved `loadData`/`runLoaders` ran only at mount, and the app has no realtime subscription, no unstable effect dep, and no timer outside the `activeWorkout`-gated ones. Traced to **`executeWithRetry` inside `PostgrestBuilder`** (`@supabase/postgrest-js` 2.106.1, vendored under supabase-js of the same version): on a network-level fetch failure it retries internally, so a single `await` from our code issues several HTTP requests before it ever resolves. Defaults, from `src/types/common/common.ts`: `retryEnabled = builder.retry ?? true` (**on by default**), `DEFAULT_MAX_RETRIES = 3`, `getRetryDelay = min(1000 * 2**attempt, 30000)` → **1s, 2s, 4s**, and `RETRYABLE_METHODS = ['GET','HEAD','OPTIONS']` — reads only, writes are never retried. That matches every symptom: the ~1/s cadence is the first retry, only reads repeat, and it is bounded (~7s per read), not an infinite loop.
  - **Decision: leave the client's default retry as-is; do not tune `.retry()`.** It causes no duplicate toasts and nothing user-visible — the `alreadyToasted` (`loadToastedRef`) guards in Dashboard/FoodLog collapse a whole failed batch to one toast regardless of how many times the client retries underneath. Known consequence to keep in mind: because our `await` only resolves after the retries are exhausted, **offline error toasts surface ~7s late rather than instantly**. If that ever needs to be faster, the lever is per-query `.retry(false)` / a lower max — not a change to our error handling.
  - Two fixes fell out of this investigation and close with it: **FoodLog double-toast on mount** (two batches run on mount — the `[date]` effect and the `[]` effect — so the per-batch guard still fired twice; now a once-per-mount `loadToastedRef`), and **`UndoToast` rendering a dead Undo button** when no `onUndo` is passed (~78 call sites) — tapping it threw `toast.onUndo is not a function`. Pre-existing, shipped since the write-toast work in 1f635b2; fixed in both `UndoToast` (optional prop) and `App.js` (pass `null` instead of an always-truthy wrapper).
